...
package org.apache.flink.streaming.connectors.kinesis2;
...
import org.apache.flink.streaming.connectors.kinesis2.internals.KinesisDataFetcher;
...
import org.apache.flink.streaming.connectors.kinesis2.serialization.KinesisDeserializationSchemaWrapper;
...
public class FlinkKinesisConsumer<T> extends RichParallelSourceFunction<T> implements
	...
	private static final String DEFAULT_CONSUMER_PREFIX = "Enhanced.Fanout.Flink.Consumer.";
	...
	/**
	 * The names of the Kinesis enhanced-fanout consumer names that will be registered to aws.
	 */
	...
	public FlinkKinesisConsumer(String stream, DeserializationSchema<T> deserializer, Properties configProps) {
		this(stream, DEFAULT_CONSUMER_PREFIX + stream, deserializer, configProps);
	}
	...
	public FlinkKinesisConsumer(List<String> streams, KinesisDeserializationSchema<T> deserializer, Properties configProps) {
		this(streams, streams.stream().map(stream -> DEFAULT_CONSUMER_PREFIX + stream).collect(Collectors.toList()), deserializer, configProps);
	}
	 ...
	 *
	 * @param stream       The single AWS Kinesis stream to read from.
	 * @param consumer     The enhanced fanout consumer name you want to register.
	 * @param deserializer The deserializer used to convert raw bytes of Kinesis records to Java objects (without key).
	 * @param configProps  The properties used to configure AWS credentials, AWS region, and initial starting position.
	 */
	public FlinkKinesisConsumer(String stream, String consumer, DeserializationSchema<T> deserializer, Properties configProps) {
		this(stream, consumer, new KinesisDeserializationSchemaWrapper<>(deserializer), configProps);
	}
	 ...
	 * @param configProps  The properties used to configure AWS credentials, AWS region, and initial starting position.
	 */
	public FlinkKinesisConsumer(String stream, String consumer, KinesisDeserializationSchema<T> deserializer, Properties configProps) {
		this(Collections.singletonList(stream), Collections.singletonList(consumer), deserializer, configProps);
	}
	 ...
	 *
	 * @param streams      The AWS Kinesis streams to read from.
	 * @param consumers    The Kinesis enhanced fan-out consumer name.
	 * @param deserializer The keyed deserializer used to convert raw bytes of Kinesis records to Java objects.
	 * @param configProps  The properties used to configure AWS credentials, AWS region, and initial starting position.
	 */
	public FlinkKinesisConsumer(List<String> streams, List<String> consumers, KinesisDeserializationSchema<T> deserializer, Properties configProps) {
		checkNotNull(streams, "streams can not be null");
		checkNotNull(consumers, "consumers can not be null");
		checkArgument(streams.size() == consumers.size(), "The Kinesis stream and consumer must have the same size.");
	...
	public void run(SourceContext<T> sourceContext) throws Exception {
		...
		KinesisDataFetcher<T> fetcher = createFetcher(streams, consumers, sourceContext, getRuntimeContext(), configProps, deserializer);
	...
	protected KinesisDataFetcher<T> createFetcher(
		...
		return new KinesisDataFetcher<>(streams, consumers, sourceContext, runtimeContext, configProps, deserializationSchema, shardAssigner, periodicWatermarkAssigner, watermarkTracker);
	}
